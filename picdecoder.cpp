...
#include <SD.h> //使用SD库打开文件、获取位置和偏移、关闭文件
File sdfile;  //将SD库File类型文件定义为全局变量
...
//(ex,ey) :结束显示的坐标点
//图片会在开始和结束的坐标点范围内显示
bool AI_LoadPicFile(u8* filename, u16 sx, u16 sy, u16 ex, u16 ey,void (*screenDrawPixel)(short,short,u16))
{
    //窗口大小
    ...
    {
        funcret = bmpDecode(filename, screenDrawPixel);
    }
    else if (fileTpe == JPGTYPE)//得到JPG/JPEG图片
    {
        funcret = jpgDecode(filename, screenDrawPixel);
    }
    else
    {
        sdfile.close();
